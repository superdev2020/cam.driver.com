...
import { ActivityIndicator, ScrollView, Switch, Text, View, Button } from 'react-native';
...
export default class MapPage2 extends React.Component {
  ...
  _startWatchingLocationWithPolyfill = () => {
    // alert('_startWatchingLocationWithPolyfill');
  ...
  renderProviderStatus = () => {
    ...
    return <Button onPress={this._checkProviderStatus} title="Stop Watching Location"/>;
  };
  ...
  renderLocationMapButton() {
    return (
      <View>
        <Button onPress={this._goToBackgroundLocationMap} title="Background location map" />
        <Button onPress={this._goToGeofencingMap} title="Geofencing map" />
      </View>
    );
  }

  componentDidMount() {
    Location.startLocationUpdatesAsync("MyLocation")
  }
